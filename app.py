...
def rilis_order_import():
    ...
    try:
        ...
        flash(
            f"File '{filename}' diterima dan sedang diproses di background (upload #{upload_id}). "
            "Refresh halaman ini beberapa saat lagi untuk melihat progres & hasilnya.",
            "success",
        )
        return redirect(url_for("rilis_order_page"))
        ...
        logger.exception("Unexpected error saat memulai upload Rilis Order")
...
def api_rilis_order_detail(row_id):
    try:
        detail = rilis_order_db_service.get_rilis_order_detail(row_id)
    except Exception as e:
        logger.exception("Gagal mengambil detail Rilis Order id=%s", row_id)
        return jsonify({"ok": False, "error": f"{type(e).__name__}: {e}"}), 500
    if not detail:
        return jsonify({"ok": False, "error": "Data tidak ditemukan."}), 404
    return jsonify({"ok": True, "data": detail})
